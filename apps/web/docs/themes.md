...
1. **Create theme CSS file** → `@/styles/themes/your-theme-name.css` with `[data-theme='your-theme-name']`
2. **Import theme** → Add `@import` to `@/styles/theme.css`
3. **Register theme** → Add to `THEMES` array in `@/components/themes/theme.config.ts`
4. **Add fonts (if needed)** → Import fonts in `@/components/themes/font.config.ts` if using custom Google Fonts
5. **Set as default (optional)** → Update `DEFAULT_THEME` in `@/components/themes/active-theme.tsx`
...
1. Open `@/components/themes/active-theme.tsx`
...
All themes are located in `@/styles/themes/` directory. Each theme is a complete, self-contained CSS file that defines all design tokens for both light and dark modes.
...
Create a new file in `@/styles/themes/` with a descriptive name (use kebab-case):

```bash
@/styles/themes/your-theme-name.css
```
...
Add your theme import to `@/styles/theme.css`:
...
Add your theme to the `THEMES` array in `@/components/themes/theme.config.ts`:
...
**File:** `@/components/themes/font.config.ts`
...
1. ✅ `@/styles/themes/your-theme-name.css` - Create theme file with `[data-theme]` attribute
2. ✅ `@/styles/theme.css` - Import your theme file
3. ✅ `@/components/themes/theme.config.ts` - Add theme to `THEMES` array
4. ⚠️ `@/components/themes/font.config.ts` - Add fonts only if needed
5. ⚠️ `@/components/themes/active-theme.tsx` - Set as default only if desired
...
See `@/styles/themes/claude.css` for a complete example with all tokens defined.
...
- Check that the file is imported in `@/styles/theme.css`
...
Edit `@/components/themes/theme.config.ts` and update the `DEFAULT_THEME` constant:
...
**Tip:** Check `@/components/themes/font.config.ts` first - many fonts may already be available!

### Font Loading Process

1. **Import the font** in `@/components/themes/font.config.ts`:
...
- **Complete theme example**: `@/styles/themes/claude.css`
- **Theme aggregator**: `@/styles/theme.css`
- **Theme selector component**: `@/components/themes/theme-selector.tsx`
- **Theme provider**: `@/components/themes/active-theme.tsx`
- **Theme configuration** (includes default theme): `@/components/themes/theme.config.ts`
- **Font configuration**: `@/components/themes/font.config.ts`